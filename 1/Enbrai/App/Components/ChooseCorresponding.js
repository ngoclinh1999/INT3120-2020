  ...
  const handleItemSelected = (data, index) => {
    ...
    if (listSelected.length > 1) {
      if (
        ...
        listSelected[listSelected.length - 2]
      ) {
        changeColorItemSelected('white', '#6E6E6E', 'white', index);
      } else if (findDataIndex == 0) {
        if (
          listCorresponding[findDataIndex] ==
          listCorresponding[findDataIndex + 1]
        ) {
          setTimeout(
            changeColorItemSelected('green', '#6E6E6E', 'green', index),
            500,
          );
    ...
    <View style={{flex: 1}}>
      <View
        ...
        <Text style={{color: '#6E6E6E', fontSize: 18}}>
          Chọn các từ và bản dịch theo cặp
        </Text>
      </View>
      <View
        style={{
      ...
      <View
        ...
        alignSelf="center"
        alignItems="center"
        style={{width: widthW * 0.7, borderRadius: 5}}>
        <ProgressBarAndroid
